...
export default{
    ...
    computed: {
        currentMail() {
            return this.$store.getters['mail/getMail'](this.openMailId)
        },
        labelColor() {
            return (label) => {
                const tags = this.$store.state.mail.mailTags;
        ...
        currentMailLabels: {
            get() {
                return this.$store.getters['mail/getMail'](this.openMailId).labels;
            },
            set(value) {
                if(Array.isArray(value)){
                    const payload = { mailId: this.openMailId, value: value }
                    this.$store.dispatch('mail/updateMailLabels', payload)
                }
            }
        }
    },
    ...
    updated() {
        if(this.currentMail.unread && this.isSidebarActive) this.$store.dispatch('mail/updateMailUnread', {mails: [this.openMailId], unread: false});
    },
}
</script>